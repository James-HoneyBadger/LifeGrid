...
class StatisticsCollector:
    ...
    def collect(
        ...
        alive_cells = np.count_nonzero(grid)
        ...
        if self.track_changes and self._previous_grid is not None:
            ...
            births = np.sum((self._previous_grid == 0) & (grid != 0))
            # Deaths: cells that were alive and are now dead
            deaths = np.sum((self._previous_grid != 0) & (grid == 0))
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'duration': self.statistics[-1].timestamp if self.statistics else 0,
...
class StatisticsExporter:
    ...
    def generate_plots(
        ...
        Args:
            ...
            plot_types: Types of plots to generate (density, births_deaths, stability)
