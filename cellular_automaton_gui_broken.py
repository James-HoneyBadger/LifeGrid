...
class CellularAutomatonGUI:
    ...
    def __init__(self, root):
        ...
        # Modern color scheme - lighter for visibility
        self.colors = {
            "bg": "#2b2b2b",  # Lighter dark gray
            "fg": "#f0f0f0",  # Bright text
            "text_secondary": "#a0a0a0",  # Medium gray
            "accent": "#0078d4",
            "accent_hover": "#1984d8",
            "secondary": "#3c3c3c",  # Lighter secondary
            "border": "#555555",  # Visible borders
            "success": "#4caf50",
            "warning": "#ff9800",
            "danger": "#f44336",
            "canvas_bg": "#1a1a1a",  # Very dark for canvas
        }
    ...
    def setup_styles(self):
        ...
        style = ttk.Style()
        # Try 'alt' theme first, fall back to 'default' if not available
        try:
            style.theme_use("alt")
        except Exception:
        ...
        style.map(
            "Danger.TButton",
            background=[("active", "#f55a4e"), ("pressed", "#e53935")],
        )
    ...
    def create_widgets(self):
        ...
        sidebar = tk.Frame(
            main_container, bg=self.colors["secondary"], width=320
        )
        sidebar.grid(
            row=0, column=0, sticky=(tk.N, tk.S, tk.W), padx=10, pady=10
        )
        ...
        title_label = tk.Label(
            sidebar,
            text="⚡ Cellular Automaton Studio",
        ...
        ttk.Button(
            file_frame, text="💾 Save", command=self.save_pattern, width=13
        ).grid(row=0, column=0, padx=(0, 4))
        ttk.Button(
            file_frame, text="📁 Load", command=self.load_saved_pattern, width=13
        ).grid(row=0, column=1)
        ...
        ttk.Button(export_frame, text="🖼️ PNG", command=self.export_png, width=13).grid(
            row=0, column=0, padx=(0, 4)
        )
        ttk.Button(export_frame, text="🎬 GIF", command=self.export_gif, width=13).grid(
            row=0, column=1
        )

        # Custom rules
        ttk.Label(sidebar, text="CUSTOM RULES (B/S)").grid(
            row=row, column=0, columnspan=2, sticky=tk.W, pady=(10, 5)
        )
        row += 1

        self.rules_var = tk.StringVar(value="B3/S23")
        rules_frame = ttk.Frame(sidebar)
        rules_frame.grid(
            row=row, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=(0, 15)
        )
        row += 1
        ...
        self.rules_entry.grid(row=0, column=0, sticky=(tk.W, tk.E), padx=(0, 4))
        rules_frame.columnconfigure(0, weight=1)

        ttk.Button(
            rules_frame, text="Apply", command=self.apply_custom_rules, width=8
        ).grid(row=0, column=1)

        # Grid size
        ttk.Label(sidebar, text="GRID SIZE").grid(
            row=row, column=0, columnspan=2, sticky=tk.W, pady=(10, 5)
        )
        row += 1
        ...
        # Custom size inputs
        size_custom_frame = ttk.Frame(sidebar)
        size_custom_frame.grid(
            row=row, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=(0, 15)
        )
        row += 1

        ttk.Label(size_custom_frame, text="W:").grid(
        ...
        self.width_spinbox.grid(row=0, column=1, padx=(0, 8))

        ttk.Label(size_custom_frame, text="H:").grid(
        ...
        self.height_spinbox.grid(row=0, column=3, padx=(0, 8))
        ...
        # Drawing tools
        ttk.Label(sidebar, text="DRAW TOOLS").grid(
            row=row, column=0, columnspan=2, sticky=tk.W, pady=(10, 5)
        )
        row += 1

        self.draw_mode_var = tk.StringVar(value="toggle")
        draw_modes_frame = ttk.Frame(sidebar)
        draw_modes_frame.grid(row=row, column=0, columnspan=2, sticky=tk.W, pady=(0, 8))
        row += 1
        ...
        ttk.Label(sidebar, text="Symmetry:").grid(
        ...
        # Speed control
        ttk.Label(sidebar, text="SPEED").grid(
            row=row, column=0, columnspan=2, sticky=tk.W, pady=(10, 5)
        )
        row += 1
        ...
        # Canvas area
        canvas_container = ttk.Frame(main_container, style="Card.TFrame")
        canvas_container.grid(
            row=0, column=1, sticky=(tk.N, tk.S, tk.E, tk.W), padx=(0, 10), pady=0
        )
        ...
        fps_label = ttk.Label(
            status_frame, textvariable=self.fps_var, foreground=self.colors["accent"]
        )
        fps_label.pack(side=tk.RIGHT, padx=10, pady=8)
